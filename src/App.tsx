...
export function App() {
  ...
  const transactions = useMemo(
    () => {
      const unsortedTransactions = paginatedTransactions?.data ?? transactionsByEmployee ?? null;
      
      // Sort transactions by date in descending order
      return unsortedTransactions?.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) ?? null;
    },
    [paginatedTransactions, transactionsByEmployee]
  );
